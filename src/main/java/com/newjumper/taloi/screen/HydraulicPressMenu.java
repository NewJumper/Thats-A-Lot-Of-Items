...
public class HydraulicPressMenu extends AbstractContainerMenu {
    ...
    public HydraulicPressMenu(int pContainerId, Inventory pInventory, BlockEntity pBlockEntity, ContainerData pContainerData) {
        super(TaloiMenuTypes.HYDRAULIC_PRESS_MENU.get(), pContainerId);
    ...
    public boolean stillValid(@NotNull Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, TaloiBlocks.ALPHA_HYDRAULIC_PRESS.get()) ||
               stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, TaloiBlocks.BETA_HYDRAULIC_PRESS.get());
    }
