...
public class ConstructingCategory implements IRecipeCategory<ConstructingRecipe> {
    ...
    public ConstructingCategory(IGuiHelper guiHelper, int progress) {
        this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 78, 54);
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(TaloiBlocks.ALPHA_CONSTRUCTOR.get()));
    ...
    public RecipeType<ConstructingRecipe> getRecipeType() {
        return RecipeType.create(ThatsALotOfItems.MOD_ID, "constructing", ConstructingRecipe.class);
    }

    @Override
    public Component getTitle() {
        return Component.translatable("container." + ThatsALotOfItems.MOD_ID + ".ac");
    }
