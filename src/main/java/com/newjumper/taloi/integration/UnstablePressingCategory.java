...
public class UnstablePressingCategory implements IRecipeCategory<UnstablePressingRecipe> {
    ...
    public UnstablePressingCategory(IGuiHelper guiHelper, int progress) {
        this.background = guiHelper.createDrawable(TEXTURE, 0, 163, 98, 55);
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(TaloiBlocks.UNSTABLE_HYDRAULIC_PRESS.get()));
    ...
    @Override
    public RecipeType<UnstablePressingRecipe> getRecipeType() {
        return RecipeType.create(ThatsALotOfItems.MOD_ID, "unstable_pressing", UnstablePressingRecipe.class);
    }

    @Override
    public Component getTitle() {
        return Component.translatable("container." + ThatsALotOfItems.MOD_ID + ".uhp");
    }
    ...
    public void draw(UnstablePressingRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        this.progressUpper.getUnchecked((time - 8) / 3).draw(stack, 3, 0);
        this.progressLower.getUnchecked((time - 8) / 3).draw(stack, 3, 38);
